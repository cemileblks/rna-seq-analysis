...
treatment <- c()
for (i in 1:seq_along(split_names)) {
...
# Rlog
png("results/rlog_boxplot.png", width = 1000, height = 600)
...
qplot(pca$x[, 1],
  ...
  main = "2D PCA of rlog-normalised Counts"
) + scale_colour_manual(
  values = c(
    "BiotinPos" = "#00FFFF",
    "BiotinNeg" = "#FF0000"
...
png("results/PCA_rlog_counts.png", width = 760, height = 460, res = 150)
...
png(filename = "results/heatmap_candidates.png", width = 350, height = 720)
...
top10_df$Gene_Combined <- rownames(top10_df)

# Extract Ensembl ID from 'Gene_Name', which is in format "ENSMUSG..._GeneName"
split_cols <- strsplit(top10_df$Gene_Combined, "_")
top10_df$Ensembl_ID <- sapply(split_cols, `[`, 1)
top10_df$Gene <- sapply(split_cols, `[`, 2)

# Reorder columns to put gene name first
top10_df <- top10_df[, c("Ensembl_ID", "Gene", "log2FoldChange", "pvalue", "padj")]

# Export to CSV
write.csv(top10_df, file = "results/top10_DE_genes.csv", row.names = FALSE)
...
paper_genes <- c(
  "Tnfrsf9", "Entpd1", "Lag3", "Pdcd1", "Il2ra",
...
png("results/volcano_test.png", width = 800, height = 800)
plot(volcano_test)
dev.off()
...
png("results/NES_barplot.png", width = 8, height = 6, units = "in", res = 300)
